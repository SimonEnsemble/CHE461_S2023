...
```math
\begin{equation}
    A\frac{dh}{dt}=u(t) + d(t) - \frac{1}{R} h(t) \quad \text{for } t \geq 0, 0 \leq h < h_{max}
\end{equation}
```
with constant parameters defined below:
*  $A$ [m$^2$]: cross-sectional area of tank from helicopter view
*  $R$ [min / m$^2$]: resistance to flow in exit line
*  $h_{max}$ [m]: the height of the tank. if the liquid level rises above `h_max`, the tank overflows.
"
...
* we wish to maintain $h$ at $h_{sp}=\bar{h}=4$ [m]. 
"
...
md"a block diagram showing the simplified feedback control loop is below. 

!!! note
	we simplify by neglecting the current-to-pressure transducer, sensor, and valve dynamics and amalgamating their gains into the control law.
"
...
now consider the *closed-loop* response when a level controller, a P-controller with controller gain $|K_c|=2.0$, is active. simulate the closed-loop response $h(t)$ to the step input in the disturbance $d(t)$ when the P-controller is actively counteracting this disturbance.
...
!!! note
	the notation \"r\" in $G_r(s)$ indicates this transfer function describes the closed-loop **r**egulator response. 
