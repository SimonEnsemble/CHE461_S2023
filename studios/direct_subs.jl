...
!!! note
	marginal stability is at the boundary of stability and instability---eg. the impulse response of a marginally stable system will present undamped oscillations (constant amplitude).
...
🦖 simulate the closed-loop response to a unit step input in the disturbance variable (with $K_c:=K_c^\prime$).
* construct `D★`
* construct `Y★`
...
md"🦖 modify the cell above where you specify the controller transfer function, `G_c`. instead, set `G_c` according to a PID controller with $K_c$, $\tau_I$, and $\tau_D$ set via these ZN-tuning rules. observe the simulated step response to the disturbance with this PID controller. does the controller reasonably perform, in comparison to no control as a baseline?
...
!!! warning
	turn in your studio with the `G_c` set according to the ZN-tuning rules.
"
